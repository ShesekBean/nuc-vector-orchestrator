...
var { RtsV7Handler } = require("./rtsV7Handler.js");
var { Sessions } = require("./sessions.js");
var { Settings } = require("./settings.js");
var { TYPE } = require("./stack.js");
const { getNetworkIp } = require("../utils/ip.js");
const OTA_VERSIONS_URL = "http://vectorfirmware.api.anki.bot/vicos/";
...
let osType = OSTypes.PROD
let isAutoSelectOTA = true
let selectedTimezone = null;
let currentLocation = "San Francisco, California, United States"
...
function fetchLatestOta() {
  ...
  switch (osType) {
    ...
    let url = OTA_VERSIONS_URL + os + "/latest.ota"
...
function filterOtaVersions(otaVersions, osType) {
  switch (osType) {
    case OSTypes.OSKR:
      return otaVersions.filter(v => v.toLowerCase().includes("oskr"));
    case OSTypes.DEV:
      return otaVersions.filter(v => v.toLowerCase().includes("d"));
    case OSTypes.PROD:
      return otaVersions.filter(
        v =>
          !v.toLowerCase().includes("oskr") &&
...
function setupOTAFiles() {
  ...
  .then((data) => {
    ...
    const otaVersions = filterOtaVersions(data, osType);
    console.log("otaVersions", otaVersions);
    setPhase("containerOta");
    ...
    // One URL present
    else if (otaVersions.length == 1) {
      $("#containerOtaSelection").addClass("vec-hidden");
      $("#otaUpdate").removeClass("vec-hidden");
      _otaEndpoint = otaVersions[0];
      doOta({ url: otaVersions[0] });
    } else {
      let urlViews = "";
      otaVersions.forEach((filename, index) => {
        urlViews += generateOtaFileRow(index, filename);
      });
      ...
      $(".vec-ota-row").click(function () {
        ...
        const url = OTA_VERSIONS_URL + otaVersions[selectedUrl]
        _otaEndpoint = url;
...
function getOtasPresent() {
  return fetch("/api/vicos")
  .then(res => res.text())
  .then(html => {
...
function generateOtaFileRow(index, filename) {
  const img = "/static/images/fontawesome/cloud-download-alt-solid.svg";
  return `
    <div data-value="${index}" class="row vec-ota-row">
      <div class="col-md-2">
        <img class="vec-ota-type" src="${img}" />
      </div>
      <div class="vec-ota-name col-md-10">
        <div class="vec-ota-name col-md-12">${filename}</div>
      </div>
    </div>
  `;
}
...
function getOtaUrl() {
  if ("otaUrl" in urlParams) {
    console.log("urlParams[\"otaUrl\"]", urlParams["otaUrl"])

    return urlParams["otaUrl"];
  } else {
    console.log("_otaEndpoint", _otaEndpoint)
    return _otaEndpoint;
  }
}
...
function setPhase(phase) {
  ...
  } else if (phase == "containerEnterPin") {
    const el = document.getElementById("osCheckboxContainer");
    if (_version == 2) {
...
function doOta({ url = getOtaUrl() }) {
  console.log("do ota")
  if (_version == 2) {
    console.log("do ota url", url)
    rtsHandler.doOtaStart(url).then((msg) => {
      console.log("ota success");
    }).catch((err) => {
      console.log("ota failure", err);
...
$(document).ready(function () {
  ...
  $.getJSON("/static/data/settings.json", function (json) {
    try {
      ...
      console.error("fetch something", error);
...
async function doPasswordReset(inputEmail) {
  return await fetch('/api/v1/reset_user_password', {
    method: "POST",
    body: JSON.stringify({
      email: inputEmail,
    }).then(data => data.json()),
...
function updateStatusBox(m) {
  ...
  isAutoSelectOTA = document.getElementById("checkboxAutoSelectOTA").checked;
  if (m.value.version.toLowerCase().includes("oskr") || m.value.version.toLowerCase().includes("something"))  {
    osType = OSTypes.OSKR
  } else if (m.value.version.toLowerCase().includes("d.")) {
    osType = OSTypes.DEV
  } else {
    osType = OSTypes.PROD
  }
...
function HandleHandshake(version) {
  ...
  rtsHandler.onEncryptedConnection(function () {
    ...
    console.log("[v" + _version + "] " + vecBle.bleName.split(" ")[1])
  ...
  rtsHandler.onOtaProgress(function (value) {
    if (value.status == 2) {
      ...
      // todo: handle failure
      console.log("Failed to download OTA")
    }
  });
...
$("#openStratus").click(function (e) {
    e.preventDefault();
      fetch("/api/stratus-url")
      .then(res => res.json())
      .then(data => {
...
$("#btnConnectCloud").click(function () {
  ...
  doCloudLogin(cloudUsername, cloudPassword).then(async (data) => {
    const { session: { session_token = "" } = {} } = await data.json()

    cloudSession.sesionToken = session_token;

    rtsHandler.doAnkiAuth(cloudSession.sesionToken).then(function (msg) {
      console.log("doAnkiAuth", msg)
...
document.addEventListener("DOMContentLoaded", function () {
  const select = document.getElementById("selectTimeZone");
  if (!select) return;
  ...
  fetch("/static/data/timezones.json")
    ...
    .then(response => {
      if (!response.ok) throw new Error("Failed to load timezones.json");
    ...
    .then(timezones => {
      timezones.forEach(zone => {
        ...
        // "America/Anchorage",
        // Format this to look like "City/GMT+00"
        const zoneTime = zone.text.split(" ")[0]
        zone.utc.forEach(utc => {
          zoneName = utc + " " + zoneTime
          const zone = {
            name: zoneName,
            value: utc,
          };
      ...
      zoneList.forEach(zoneObject => {
        zoneMap.set(zoneObject.name, zoneObject.value)
      })

      zoneMap.forEach((value, key) => {
        const option = document.createElement("option");
        option.textContent = key;
        option.value = value;

        if (value.includes(userZone)) {
...
$("#btnFinishSetup").click(function () {
  ...
  let is24hour = $("#selectTimeFormat").val() == "24";
            ...
            .doSdk(
              ...
              JSON.stringify({
                settings: {
                  ...
                  clock_24_hour: is24hour,
                  default_location: currentLocation
                },
              })
            )
...
},{"../utils/ip.js":18,"./clad.js":4,"./rtsCliUtil.js":7,"./rtsV2Handler.js":8,"./rtsV3Handler.js":9,"./rtsV4Handler.js":10,"./rtsV5Handler.js":11,"./rtsV6Handler.js":12,"./rtsV7Handler.js":13,"./sessions.js":14,"./settings.js":15,"./stack.js":16,"./vectorBluetooth.js":17}],6:[function(require,module,exports){
//Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details.
//
...
Anki.Vector.ExternalComms.RtsConnection = class extends Clad {
  ...
  unpackStructure(tag, buffer) {
    ...
    switch (tag) {
      ...
      case Anki.Vector.ExternalComms.RtsConnectionTag.RtsConnection_6:
        ...
        this.value = ret;
        return ret;
      case Anki.Vector.ExternalComms.RtsConnectionTag.RtsConnection_7:
        ret = new Anki.Vector.ExternalComms.RtsConnection_7();
        ret.unpack(buffer);
        this.value = ret;
        return ret;
      default:
  ...
  getRtsConnection_7() {
    if (
      this.tag != Anki.Vector.ExternalComms.RtsConnectionTag.RtsConnection_7
    ) {
      return null;
    }
    return this.value;
  ...
  static NewRtsConnectionWithRtsConnection_7(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection();
    m._tag = Anki.Vector.ExternalComms.RtsConnectionTag.RtsConnection_7;
    m.value = value;
    return m;
  }
};
...
// MESSAGE RtsStatusResponse_7
Anki.Vector.ExternalComms.RtsStatusResponse_7 = class extends Clad {
...
// MESSAGE RtsWifiScanResult_7
Anki.Vector.ExternalComms.RtsWifiScanResult_7 = class extends Clad {
  constructor(authType, signalStrength, wifiSsidHex, hidden, provisioned) {
    super();
    this.authType = authType;
    this.signalStrength = signalStrength;
...
// MESSAGE RtsWifiScanResponse_7
Anki.Vector.ExternalComms.RtsWifiScanResponse_7 = class extends Clad {
  constructor(statusCode, scanResult) {
    super();
    this.statusCode = statusCode;
    this.scanResult = scanResult;
  ...
  unpack(buffer) {
    ...
    this.scanResult = [];
    for(let i = 0; i < scanResultLength; i++) {
      let scanResultNew = new Anki.Vector.ExternalComms.RtsWifiScanResult_7(null);
      scanResultNew.unpackFromClad(cladBuffer);
      this.scanResult.push(scanResultNew);
    }
  }
...
// MESSAGE RtsWifiConnectResponse_7
Anki.Vector.ExternalComms.RtsWifiConnectResponse_7 = class extends Clad {
  constructor(wifiSsidHex, wifiState, connectResult) {
    super();
    this.wifiSsidHex = wifiSsidHex;
    this.wifiState = wifiState;
  ...
  type() {
    return "RtsWifiConnectResponse_7";
  }
...
// MESSAGE RtsCloudSessionRequest_7
Anki.Vector.ExternalComms.RtsCloudSessionRequest_7 = class extends Clad {
  constructor(sessionToken, clientName, appId) {
    super();
    this.sessionToken = sessionToken;
    this.clientName = clientName;
  ...
  type() {
    return "RtsCloudSessionRequest_7";
  }
...
// UNION RtsConnection_7
Anki.Vector.ExternalComms.RtsConnection_7Tag = Object.freeze({
  Error:0x0,                      
  RtsConnRequest:0x1,             
  RtsConnResponse:0x2,            
  RtsNonceMessage:0x3,            
  ...
  RtsChallengeSuccessMessage:0x5, 
  RtsWifiConnectRequest:0x6,      
  RtsWifiConnectResponse_7:0x7,   
  RtsWifiIpRequest:0x8,           
  RtsWifiIpResponse:0x9,          
  RtsStatusRequest:0xa,           
  RtsStatusResponse_7:0xb,        
  RtsWifiScanRequest:0xc,         
  RtsWifiScanResponse_7:0xd,      
  RtsOtaUpdateRequest:0xe,        
  RtsOtaUpdateResponse:0xf,       
  RtsCancelPairing:0x10,          
  RtsForceDisconnect:0x11,        
  ...
  RtsSdkProxyRequest:0x22,        
  RtsSdkProxyResponse:0x23,       
  RtsErrorResponse:0x24,          
  INVALID:0xFF                    
});

Anki.Vector.ExternalComms.RtsConnection_7 = class extends Clad {
  constructor() {
    super();
    this._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.INVALID;
  }
  
  get tag() {
    if(this._tag == null) {
      return Anki.Vector.ExternalComms.RtsConnection_7Tag.INVALID;
    }
    return this._tag;
  }
  
  get size() {
    if(this._tag == null || this._tag == Anki.Vector.ExternalComms.RtsConnection_7Tag.INVALID) {
      return 1;
    }
    return 1 + this.value.size;
  }
  ...
  pack() {
    if(this._tag == Anki.Vector.ExternalComms.RtsConnection_7Tag.INVALID) {
      return null;
    }
    let buffer = new Uint8Array(this.size);
    // add tag
  ...
  unpackStructure(tag, buffer) {
    let ret = null;
    switch(tag) {
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.Error:
      ret = new Anki.Vector.ExternalComms.Error();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsConnRequest:
      ret = new Anki.Vector.ExternalComms.RtsConnRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsConnResponse:
      ret = new Anki.Vector.ExternalComms.RtsConnResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsNonceMessage:
      ret = new Anki.Vector.ExternalComms.RtsNonceMessage();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsChallengeMessage:
      ret = new Anki.Vector.ExternalComms.RtsChallengeMessage();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsChallengeSuccessMessage:
      ret = new Anki.Vector.ExternalComms.RtsChallengeSuccessMessage();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiConnectRequest:
      ret = new Anki.Vector.ExternalComms.RtsWifiConnectRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiConnectResponse_7:
      ret = new Anki.Vector.ExternalComms.RtsWifiConnectResponse_7();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiIpRequest:
      ret = new Anki.Vector.ExternalComms.RtsWifiIpRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiIpResponse:
      ret = new Anki.Vector.ExternalComms.RtsWifiIpResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsStatusRequest:
      ret = new Anki.Vector.ExternalComms.RtsStatusRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsStatusResponse_7:
      ret = new Anki.Vector.ExternalComms.RtsStatusResponse_7();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiScanRequest:
      ret = new Anki.Vector.ExternalComms.RtsWifiScanRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiScanResponse_7:
      ret = new Anki.Vector.ExternalComms.RtsWifiScanResponse_7();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaUpdateRequest:
      ret = new Anki.Vector.ExternalComms.RtsOtaUpdateRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaUpdateResponse:
      ret = new Anki.Vector.ExternalComms.RtsOtaUpdateResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCancelPairing:
      ret = new Anki.Vector.ExternalComms.RtsCancelPairing();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsForceDisconnect:
      ret = new Anki.Vector.ExternalComms.RtsForceDisconnect();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAck:
      ret = new Anki.Vector.ExternalComms.RtsAck();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiAccessPointRequest:
      ret = new Anki.Vector.ExternalComms.RtsWifiAccessPointRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiAccessPointResponse:
      ret = new Anki.Vector.ExternalComms.RtsWifiAccessPointResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSshRequest:
      ret = new Anki.Vector.ExternalComms.RtsSshRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSshResponse:
      ret = new Anki.Vector.ExternalComms.RtsSshResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaCancelRequest:
      ret = new Anki.Vector.ExternalComms.RtsOtaCancelRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsLogRequest:
      ret = new Anki.Vector.ExternalComms.RtsLogRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsLogResponse:
      ret = new Anki.Vector.ExternalComms.RtsLogResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsFileDownload:
      ret = new Anki.Vector.ExternalComms.RtsFileDownload();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiForgetRequest:
      ret = new Anki.Vector.ExternalComms.RtsWifiForgetRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiForgetResponse:
      ret = new Anki.Vector.ExternalComms.RtsWifiForgetResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCloudSessionRequest_7:
      ret = new Anki.Vector.ExternalComms.RtsCloudSessionRequest_7();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCloudSessionResponse:
      ret = new Anki.Vector.ExternalComms.RtsCloudSessionResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAppConnectionIdRequest:
      ret = new Anki.Vector.ExternalComms.RtsAppConnectionIdRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAppConnectionIdResponse:
      ret = new Anki.Vector.ExternalComms.RtsAppConnectionIdResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsResponse:
      ret = new Anki.Vector.ExternalComms.RtsResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSdkProxyRequest:
      ret = new Anki.Vector.ExternalComms.RtsSdkProxyRequest();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSdkProxyResponse:
      ret = new Anki.Vector.ExternalComms.RtsSdkProxyResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    case Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsErrorResponse:
      ret = new Anki.Vector.ExternalComms.RtsErrorResponse();
      ret.unpack(buffer);
      this.value = ret;
      return ret;
    default:
  ...
  getError() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.Error) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithError(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.Error;
    m.value = value;
    return m;
  }
  
  getRtsConnRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsConnRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsConnRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsConnRequest;
    m.value = value;
    return m;
  }
  
  getRtsConnResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsConnResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsConnResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsConnResponse;
    m.value = value;
    return m;
  }
  
  getRtsNonceMessage() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsNonceMessage) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsNonceMessage(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsNonceMessage;
    m.value = value;
    return m;
  }
  
  getRtsChallengeMessage() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsChallengeMessage) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsChallengeMessage(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsChallengeMessage;
    m.value = value;
    return m;
  }
  
  getRtsChallengeSuccessMessage() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsChallengeSuccessMessage) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsChallengeSuccessMessage(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsChallengeSuccessMessage;
    m.value = value;
    return m;
  }
  
  getRtsWifiConnectRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiConnectRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiConnectRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiConnectRequest;
    m.value = value;
    return m;
  }
  
  getRtsWifiConnectResponse_7() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiConnectResponse_7) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiConnectResponse_7(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiConnectResponse_7;
    m.value = value;
    return m;
  }
  
  getRtsWifiIpRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiIpRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiIpRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiIpRequest;
    m.value = value;
    return m;
  }
  
  getRtsWifiIpResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiIpResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiIpResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiIpResponse;
    m.value = value;
    return m;
  }
  
  getRtsStatusRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsStatusRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsStatusRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsStatusRequest;
    m.value = value;
    return m;
  }
  
  getRtsStatusResponse_7() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsStatusResponse_7) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsStatusResponse_7(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsStatusResponse_7;
    m.value = value;
    return m;
  }
  
  getRtsWifiScanRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiScanRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiScanRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiScanRequest;
    m.value = value;
    return m;
  }
  
  getRtsWifiScanResponse_7() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiScanResponse_7) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiScanResponse_7(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiScanResponse_7;
    m.value = value;
    return m;
  }
  
  getRtsOtaUpdateRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaUpdateRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsOtaUpdateRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaUpdateRequest;
    m.value = value;
    return m;
  }
  
  getRtsOtaUpdateResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaUpdateResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsOtaUpdateResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaUpdateResponse;
    m.value = value;
    return m;
  }
  
  getRtsCancelPairing() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCancelPairing) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsCancelPairing(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCancelPairing;
    m.value = value;
    return m;
  }
  
  getRtsForceDisconnect() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsForceDisconnect) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsForceDisconnect(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsForceDisconnect;
    m.value = value;
    return m;
  }
  
  getRtsAck() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAck) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsAck(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAck;
    m.value = value;
    return m;
  }
  
  getRtsWifiAccessPointRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiAccessPointRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiAccessPointRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiAccessPointRequest;
    m.value = value;
    return m;
  }
  
  getRtsWifiAccessPointResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiAccessPointResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiAccessPointResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiAccessPointResponse;
    m.value = value;
    return m;
  }
  
  getRtsSshRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSshRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsSshRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSshRequest;
    m.value = value;
    return m;
  }
  
  getRtsSshResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSshResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsSshResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSshResponse;
    m.value = value;
    return m;
  }
  
  getRtsOtaCancelRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaCancelRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsOtaCancelRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsOtaCancelRequest;
    m.value = value;
    return m;
  }
  
  getRtsLogRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsLogRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsLogRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsLogRequest;
    m.value = value;
    return m;
  }
  
  getRtsLogResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsLogResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsLogResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsLogResponse;
    m.value = value;
    return m;
  }
  
  getRtsFileDownload() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsFileDownload) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsFileDownload(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsFileDownload;
    m.value = value;
    return m;
  }
  
  getRtsWifiForgetRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiForgetRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiForgetRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiForgetRequest;
    m.value = value;
    return m;
  }
  
  getRtsWifiForgetResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiForgetResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsWifiForgetResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsWifiForgetResponse;
    m.value = value;
    return m;
  }
  
  getRtsCloudSessionRequest_7() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCloudSessionRequest_7) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsCloudSessionRequest_7(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCloudSessionRequest_7;
    m.value = value;
    return m;
  }
  
  getRtsCloudSessionResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCloudSessionResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsCloudSessionResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsCloudSessionResponse;
    m.value = value;
    return m;
  }
  
  getRtsAppConnectionIdRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAppConnectionIdRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsAppConnectionIdRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAppConnectionIdRequest;
    m.value = value;
    return m;
  }
  
  getRtsAppConnectionIdResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAppConnectionIdResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsAppConnectionIdResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsAppConnectionIdResponse;
    m.value = value;
    return m;
  }
  
  getRtsResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsResponse;
    m.value = value;
    return m;
  }
  
  getRtsSdkProxyRequest() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSdkProxyRequest) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsSdkProxyRequest(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSdkProxyRequest;
    m.value = value;
    return m;
  }
  
  getRtsSdkProxyResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSdkProxyResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsSdkProxyResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsSdkProxyResponse;
    m.value = value;
    return m;
  }
  
  getRtsErrorResponse() {
    if(this.tag != Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsErrorResponse) {
      return null;
    }
    return this.value;
  }
  
  static NewRtsConnection_7WithRtsErrorResponse(value) {
    let m = new Anki.Vector.ExternalComms.RtsConnection_7();
    m._tag = Anki.Vector.ExternalComms.RtsConnection_7Tag.RtsErrorResponse;
    m.value = value;
    return m;
  }
...
},{"./clad.js":4}],7:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
class RtsV2Handler {
  ...
  handleCli(args) {
    console.log("handle cli v2", args)
    ...
    switch (cmd) {
      ...
      case "ota-start":
        ...
        self.doOtaStart(args[1]).then((msg) => {
          console.log("doOtaStart cb v2", msg)
...
},{"./messageExternalComms.js":6,"./rtsCliUtil.js":7}],9:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
class RtsV3Handler {
  ...
  handleCli(args) {
    console.log("handle cli v3", args)
    ...
    switch (cmd) {
      ...
      case "ota-start":
        ...
        self.doOtaStart(args[1]).then((msg) => {
          console.log("doOtaStart cb v3", msg)
...
},{"./messageExternalComms.js":6,"./rtsCliUtil.js":7}],10:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
class RtsV4Handler {
  ...
  // returns whether resolved immediately
  handleCli(args) {
    console.log("handle cli v4", args)
    ...
    switch (cmd) {
      ...
      case "ota-start":
        ...
        self.doOtaStart(args[1]).then((msg) => {
          console.log("doOtaStart cb v4", msg)
          self.otaProgress.value = msg.value;
          self.hasProgressBar = false;
          self.cliResolve(msg);
        }).catch(r);
...
},{"./messageExternalComms.js":6,"./rtsCliUtil.js":7}],11:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
class RtsV5Handler {
  ...
  // returns whether resolved immediately
  handleCli(args) {
    console.log("handle cli v5", args)

    let self = this;
    let cmd = args[0];
    let r = function (msg) {
      console.log("error from ble", msg)
      self.cliResolve(msg);
    };
    let output = "";
    ...
    switch (cmd) {
      ...
      case "ota-start":
        ...
        self.doOtaStart(args[1]).then(function (msg) {
          console.log("doOtaStart cb v5", msg)
          self.otaProgress.value = msg.value;
          self.hasProgressBar = false;
          self.cliResolve(msg);
        }).catch(r);
...
},{"./messageExternalComms.js":6,"./rtsCliUtil.js":7}],12:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
class RtsV6Handler {
  ...
  // returns whether resolved immediately
  handleCli(args) {
    console.log("handle cli v6", args)
    let self = this;
    let cmd = args[0];
    let r = function (msg) {
      console.log("error from ble", msg)
    ...
    switch (cmd) {
      ...
      case "ota-start":
        ...
        self.doOtaStart(args[1]).then((msg) => {
          console.log("doOtaStart cb v6", msg)
          self.otaProgress.value = msg.value;
          self.hasProgressBar = false;
          self.cliResolve(msg);
        }).catch(r);
...
},{"./blesh.js":3,"./messageExternalComms.js":6,"./rtsCliUtil.js":7}],13:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
class RtsV7Handler {
  ...
  enterPin(pin) {
    ...
    this.send(
      Rts.RtsConnection_7.NewRtsConnection_7WithRtsAck(
        new Rts.RtsAck(Rts.RtsConnection_7Tag.RtsNonceMessage)
      )
    );
  ...
  send(rtsConn7) {
    let rtsConn = Rts.RtsConnection.NewRtsConnectionWithRtsConnection_7(
      rtsConn7
    );
    let extResponse = Rts.ExternalComms.NewExternalCommsWithRtsConnection(
      rtsConn
  ...
  receive(data) {
    ...
    if (comms.tag == Rts.ExternalCommsTag.RtsConnection) {
      switch (comms.value.tag) {
        case Rts.RtsConnectionTag.RtsConnection_7: {
          let rtsMsg = comms.value.value;

          switch (rtsMsg.tag) {
            case Rts.RtsConnection_7Tag.RtsConnRequest:
              this.onRtsConnRequest(rtsMsg.value);
              break;
            case Rts.RtsConnection_7Tag.RtsNonceMessage:
              this.onRtsNonceMessage(rtsMsg.value);
              break;
            case Rts.RtsConnection_7Tag.RtsChallengeMessage:
              this.onRtsChallengeMessage(rtsMsg.value);
              break;
            case Rts.RtsConnection_7Tag.RtsChallengeSuccessMessage:
              this.onRtsChallengeSuccessMessage(rtsMsg.value);
              break;

            // Post-connection messages
            case Rts.RtsConnection_7Tag.RtsWifiScanResponse_3:
              this.resolvePromise("wifi-scan", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsWifiConnectResponse_3:
              this.resolvePromise("wifi-connect", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsStatusResponse_7:
              this.resolvePromise("status", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsWifiForgetResponse:
              this.resolvePromise("wifi-forget", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsWifiAccessPointResponse:
              this.resolvePromise("wifi-ap", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsWifiIpResponse:
              this.resolvePromise("wifi-ip", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsCloudSessionResponse:
              for (let i = 0; i < this.onCloudAuthorizedEvent.length; i++) {
                this.onCloudAuthorizedEvent[i](rtsMsg.value);
              }
              ...
              this.resolvePromise("anki-auth", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsOtaUpdateResponse:
              this.otaProgress["value"] = rtsMsg.value;
              ...
              break;
            case Rts.RtsConnection_7Tag.RtsResponse:
              this.rejectPromise(this.waitForResponse, rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsSdkProxyResponse:
              this.resolvePromise("sdk", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsAppConnectionIdResponse:
              this.resolvePromise("connection-id", rtsMsg);
              break;
            case Rts.RtsConnection_7Tag.RtsLogResponse:
              if (rtsMsg.value.exitCode == 0) {
                this.logId = rtsMsg.value.fileId;
                this.logFile = [];
              } else {
              ...
              break;
            case Rts.RtsConnection_7Tag.RtsFileDownload:
              let chunk = rtsMsg.value;
              if (chunk.fileId == this.logId) {
                this.logFile = this.logFile.concat(chunk.fileChunk);
  ...
  onRtsConnRequest(msg) {
    ...
    if (savedSession != null) {
      ...
      // use saved session
      this.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsConnResponse(
          new Rts.RtsConnResponse(
            Rts.RtsConnType.Reconnection,
            this.keys.publicKey
          )
  ...
  onRtsChallengeMessage(msg) {
    this.send(
      Rts.RtsConnection_7.NewRtsConnection_7WithRtsChallengeMessage(
        new Rts.RtsChallengeMessage(msg.number + 1)
      )
    );
  }
  ...
  doWifiScan() {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("wifi-scan", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsWifiScanRequest(
          new Rts.RtsWifiScanRequest()
        )
      );
    });
  ...
  doWifiConnect(ssid, password, auth, timeout) {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("wifi-connect", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsWifiConnectRequest(
          new Rts.RtsWifiConnectRequest(
            RtsCliUtil.convertStrToHex(ssid),
            password,
            timeout,
  ...
  doWifiForget(ssid) {
    ...
    let p = new Promise(function (resolve, reject) {
      ...
      let hexSsid = deleteAll ? "" : RtsCliUtil.convertStrToHex(ssid);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsWifiForgetRequest(
          new Rts.RtsWifiForgetRequest(deleteAll, hexSsid)
        )
      );
    });
  ...
  doWifiAp(enable) {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("wifi-ap", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsWifiAccessPointRequest(
          new Rts.RtsWifiAccessPointRequest(enable.toLowerCase() == "true")
        )
      );
    });
  ...
  doWifiIp() {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("wifi-ip", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsWifiIpRequest(
          new Rts.RtsWifiIpRequest()
        )
      );
    });
  ...
  doStatus() {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("status", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsStatusRequest(
          new Rts.RtsStatusRequest()
        )
      );
    });
  ...
  doAnkiAuth(sessionToken) {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("anki-auth", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsCloudSessionRequest_7(
          new Rts.RtsCloudSessionRequest_7(sessionToken, "", "")
        )
      );
    });
  ...
  doOtaStart(url) {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("ota-start", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsOtaUpdateRequest(
          new Rts.RtsOtaUpdateRequest(url)
        )
      );
    });
  ...
  doOtaCancel(url) {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("ota-cancel", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsOtaCancelRequest(
          new Rts.RtsOtaCancelRequest(url)
        )
      );
    });
  ...
  doConnectionId() {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("connection-id", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsAppConnectionIdRequest(
          new Rts.RtsAppConnectionIdRequest(url)
        )
      );
    });
  ...
  doLog() {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("logs", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsLogRequest(
          new Rts.RtsLogRequest(0, [])
        )
      );
    });
  ...
  doSdk(clientGuid, id, path, json) {
    ...
    let p = new Promise(function (resolve, reject) {
      self.storePromiseMethods("sdk", resolve, reject);
      self.send(
        Rts.RtsConnection_7.NewRtsConnection_7WithRtsSdkProxyRequest(
          new Rts.RtsSdkProxyRequest(clientGuid, id, path, json)
        )
      );
    });
...
module.exports = { RtsV7Handler };

},{"./messageExternalComms.js":6,"./rtsCliUtil.js":7}],14:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
},{"./rtsCliUtil.js":7}],15:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
},{"./stack.js":16}],16:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
...
},{"./bleMessageProtocol.js":2}],18:[function(require,module,exports){
/* Copyright (c) 2019-2020 Digital Dream Labs. See LICENSE file for details. */
